...
export const useAuth = () => {
  ...
  const register = async (email: string, password: string, full_name: string) => {
    try {
      const response = await fetch('https://2d49-171-224-181-76.ngrok-free.app/api/v1/users/register', {
        method: 'POST',
        headers: {
  ...
  const login = async (username: string, password: string) => {
    try {
      ...
      const response = await fetch('https://2d49-171-224-181-76.ngrok-free.app/api/v1/auth/login', {
        method: 'POST',
        body: formData
      });
